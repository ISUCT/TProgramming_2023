...
import { PlayerFabric } from "./playerClassFabric";
...
export class GameLogic {
    ...
    public createPlayers(numberOfPlayers: number) {
        if(numberOfPlayers % 2 !== 0) {
            throw new Error('The wrong number of players');
        }
    ...
    /*private*/ public fight(p1: Player, p2: Player, p1Index: number, p2Index: number) {
        ...
        while(!p1IsDead && !p2IsDead) {
            if(playerToAttack === 1) {
                ...
                if(p1.constructor.name === 'Archer' && p1.isFireUsed && p2.step >= 1) {
                    if(p2.step > 2) {
                        p2.step = 0;
                    } else {
                        p2.burning(p2);
                ...
                if(p2.constructor.name === 'Archer' && p2.isFireUsed && p1.step >= 1) {
                    if(p1.step > 2) {
                        p1.step = 0;
                    } else {
                        p1.burning(p1);
    ...
    /*private*/ public defaultAttack(p1: Player, p2: Player, playeToAttack: number) {
        if(playeToAttack === 1) {
            p1.attack(p2);
    ...
    /*private*/ public useSpecialAttack(p1: Player, p2: Player, playerToAttack: number) {
        if(playerToAttack === 1) {
