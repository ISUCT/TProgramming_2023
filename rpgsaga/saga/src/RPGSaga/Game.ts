...
export class Game {
  ...
  private static PlayFight(fightMembers: [Player, Player]): Player {
    for (let i = 0; true; i++) {
      const playerStatus = fightMembers[i % 2].checkStatus(); // 0 либо 1 , т.е он вывод массив с 2 значениями: наложенный статус и урон от этого статуса
  ...
  // private static RandomizeList(input: Player[]): void {
  //     const buffer: Player[] = input.slice();
  //     for (let i = 0; i < input.length; i++) {
  //         const chosen: number = i + Math.floor(Math.random() * (buffer.length - i));
